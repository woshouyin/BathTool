...
public class PointAggUtil {
    ...
    public static List<Cluster> aggregate(List<Poi> points,double threshold) {
        ...
        while (!points.isEmpty()) {
            ...
            List<Poi> cluster = new ArrayList<>();
            ...
            for (int i = 0; i < points.size(); ) {
                ...
                if (DistanceCalUtils.calculateDistance(currentPoint.getLocation(),centerLatLon) <= threshold) {
                    ...
                    centerLatLon = "" + ((sumLat / cluster.size()) +"000").substring(0,9) + "," + ((sumLon / cluster.size()) +"000").substring(0,9);
                } else {
                    // 如果不在阈值内，则继续检查下一个点
                    i++;
                }
            ...
            centerLatLon = "" + ((sumLat / cluster.size()) +"000").substring(0,9) + "," + ((sumLon / cluster.size()) +"000").substring(0,9);
            Cluster clus = new Cluster(centerLatLon, "", cluster,cluster.size(),null);
            aggregate.add(clus);
        }
        return aggregate;
    }
